        ...
        return Container(
          child: new GridView.builder(
            ...
            itemBuilder: (_, int index) =>
                EachCell(Data.instance.islands[index]),
...
class EachCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return new Card(
      // key: new Key(island.id),
        child: InkWell(
      onTap: () => showBarModalBottomSheet(
        expand: true,
        context: context,
        backgroundColor: Colors.transparent,
        builder: (context, scrollController) => Image.asset(path),
      ),
      child: Container(
  ...
  List<Widget> getAnimalIcons(String id) {
    ...
    double iconSize = 32;
    if ((Data.instance.location_chickens
            .singleWhere((it) => it.id == id, orElse: () => null)) !=
        null) {
      icons.add(Image.asset('assets/images/icon_chicken.png',
          height: iconSize, width: iconSize));
    }
    if ((Data.instance.location_pigs
            .singleWhere((it) => it.id == id, orElse: () => null)) !=
        null) {
      icons.add(Image.asset('assets/images/icon_pig.png',
          height: iconSize, width: iconSize));
    }
    if ((Data.instance.location_snakes
            .singleWhere((it) => it.id == id, orElse: () => null)) !=
        null) {
      icons.add(Image.asset('assets/images/icon_snake.png',
          height: iconSize, width: iconSize));
    }
    return icons;
  }
